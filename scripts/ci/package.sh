...
composer setup-swark --no-progress --no-interaction --prefer-dist --optimize-autoloader --no-dev --working-dir=$ROOT
# swark: not needed right now
# yarn --cwd $ROOT install --immutable
# yarn --cwd $ROOT run build


# PACKAGE
package=swark-$version
...
ln -s $ROOT/default-importables $package/
...
# tar fails and returns exit code 1 if any of the links above is invalid
tar chfj $package.tar.bz2 --exclude .gitignore --exclude .gitkeep $package
echo -n -e "\nfile $package.tar.bz2 packaged"

sha512sum "$package.tar.bz2" > "$package.tar.bz2.sha512"
echo -n -e "\n sha512 for $package.tar.bz2 created"

echo "package=$package.tar.bz2" >> $GITHUB_OUTPUT

# swark: Further actions are not needed right now, so exit w/ success
exit 0

# ASSETS
swark: not needed right now
assets=swark-assets-$version
